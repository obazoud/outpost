...
Outpost is designed to be low maintenance, effective, scaleable and focus on developer experience. That's achieved by:

- Supporting tablestakes features and keeping the scope to a minimum. We don't believe it's the responsability of the producer to implement throttling, transformations or other complex logic that's costly to operate.
...
- Horizontal scaling with both single and multi-services deployments, so you can run Outpost as a single binary / process if you don't have serious event volume.
