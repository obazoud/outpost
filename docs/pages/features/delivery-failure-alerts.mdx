...
When a delivery attempt fails to deliver an event to a destination, an can be alert triggered. Alerts are scoped per destination.

Alerts can be serve to notify the owner of the destination of the failure and to optonally, auto-disabled the destination using the `ALERT_AUTO_DISABLE_DESTINATION` config.
