...
If the destination for an event is disabled—through the API, user portal, or automatically because a [failure threshold](/docs/features/delivery-failure-alerts) has been reached—the event will be discarded and cannot be retried.
