...
> We recommend setting the `MAX_DESTINATIONS_PER_TENANT` value as a low as it is appropriate for your use case to prevent abuse and performance degradation. Updating the value to a lower value later will not delete existing destinations.
...
Destinations can be registered either through the tenant-facing portal or with the API. Since each destination type uses it's own credentials and configuration, the required fields are different. Refer to the [Create Destination API](/docs/api/destinations#create-destination) for the required `config` and `credentials` fields for each destination type.
...
When using the API, you may want to build your own UI to capture user input on the destination configuration. Since each destination requires a specific configuration, the `GET /destination-types` endpoint provides a JSON schema for standardize input fields for each destination type.
